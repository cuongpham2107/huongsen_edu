                      ...
                      @foreach ($posts as $item)
                        <div class="col-md-6 col-sm-12">
                          <div class="single-blog">
                              <div class="image"> <a href="{{route('document-knowledge.show',$item->slug)}}">
                                <img src="{{Voyager::image($item->image)}}" width="360" height="240" style="object-fit: cover"
                                          alt=""></a>
                                  <a href="{{route('document-knowledge.show',$item->slug)}}" class="overlay-box"><span class="icon fa fa-link"></span></a>
                              </div>
                              <div class="blog-content">
                                  <div class="blog-title">  <span >{{date("d", strtotime($item->created_at))}}<br>
                                    {{\Carbon\Carbon::parse($item->created_at)->locale('vi-VN')->translatedFormat("F")}} </span>
                                      <h3> <a href="{{route('document-knowledge.show',$item->slug)}}">{{$item->title}}</a> </h3>
                                  </div>
                                  <p class="line-clamp">{{$item->excerpt}}
                                  </p>
                                  <a class="read-more" href="{{route('document-knowledge.show',$item->slug)}}">Xem thêm <i
